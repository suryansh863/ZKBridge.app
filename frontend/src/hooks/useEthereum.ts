  ...
  const burnWrappedBTCForBitcoin = useCallback(async (amount: string) => {
    ...
    try {
      ...
      if (!allowance || allowance < amountWei) {
        ...
        const approveAmount = amountWei * 2n; // Approve 2x to avoid frequent approvals
        approveWrappedBTC({
    ...
    try {
      ...
      const transactions: BridgeTransaction[] = initiatedEvents.map(event => {
        ...
        return {
          ...
          amount: formatUnits(event.args.amount, 8),
          fee: completedEvent ? formatUnits(completedEvent.args.fee, 8) : '0',
