...
export class BridgeService {
  ...
  async verifySourceTransaction(bridgeTxId: string): Promise<void> {
    try {
      ...
      if (bridgeTx.direction === BridgeDirection.BITCOIN_TO_ETHEREUM) {
        verificationResult = await this.bitcoinService.verifyTransaction(
          bridgeTx.sourceTxHash,
          bridgeTx.sourceAddress,
          parseInt(bridgeTx.sourceAmount)
        );
      } else {
      ...
      await this.updateBridgeConfirmations(bridgeTxId, verificationResult.details.confirmations || 0);

      // Wait for sufficient confirmations
      if (verificationResult.details.confirmations < 6) {
